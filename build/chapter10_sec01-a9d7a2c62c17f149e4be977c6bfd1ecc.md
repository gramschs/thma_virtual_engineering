...
interpretieren z als Höhenkoordinate, x und y beschrieben die horizontale Lage.
...
2. Im Menü `Tools → Other → Point picking` aktivieren (Tastenkürzel `Shift + P`).
...
daten = {
    "x": [0.00, 0.12, 0.25, 0.38, 0.51, 0.63, 0.76, 0.88, 1.00],
    "y": [0.000,  0.010,  0.020,  0.025,  0.030,  0.025,  0.015,  0.005,  0.000],
    "z": [0.000, -0.020, -0.040, -0.055, -0.065, -0.080, -0.095, -0.110, -0.120]
}
...
for i in range(len(wegpunkte) - 1):
    delta       = wegpunkte[i+1] - wegpunkte[i]
    delta_horiz = np.sqrt(delta[0]**2 + delta[1]**2)
    winkel.append(np.degrees(np.arctan2(delta[2], delta_horiz)))
...
−4° und −9°, die Bahn ist also verhältnismäßig gleichmäßig geneigt.
````
