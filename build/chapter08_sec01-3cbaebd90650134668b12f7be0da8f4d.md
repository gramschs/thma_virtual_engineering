...
```{admonition} Hinweis: Ursina läuft als eigenständiges Skript
:class: note
Alle Code-Beispiele in den Kapiteln 8 bis 10 sind eigenständige Python-Skripte,
die **nicht** im Jupyter Notebook ausgeführt werden können. Das Vorgehen ist
stets dasselbe:

1. Code in eine neue Datei kopieren, zum Beispiel `simulation.py`.
2. Im Terminal ausführen: `python simulation.py`
3. Es öffnet sich ein Fenster mit der 3D-Szene.

Die Code-Zellen im Jupyter Book dienen als Vorlagen zum Kopieren.
```
...
Diese Zeile startet den Hauptloop und muss immer am Ende stehen. Das Skript
