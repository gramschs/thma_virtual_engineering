...
Wir starten den Slicer (PrusaSlicer oder Cura) und importieren unsere
bereinigte `.ply`-Datei aus Abschnitt 4.3 über `File > Import > Import STL/OBJ/AMF/3MF`
beziehungsweise den entsprechenden Menüeintrag. Falls der Slicer das PLY-Format
nicht direkt unterstützt, exportieren wir das Mesh zuvor in CloudCompare als
`.obj` oder `.stl`.
