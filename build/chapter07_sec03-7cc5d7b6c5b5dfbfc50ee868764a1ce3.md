...
```{figure} pics/chap07_slicer_import.png
:alt: Slicer-Fenster nach dem Import der Kugelbahn mit einer Warnmeldung
  über nicht-manifold Kanten und dem angebotenen Reparatur-Dialog.
:align: center

Slicer nach dem Import: Die Warnmeldung über Non-Manifold Edges erscheint
automatisch. Der Reparatur-Dialog behebt die meisten Fehler in einem Schritt.
...
Das Modell liegt nach dem Import wahrscheinlich in der Orientierung auf der
...
**Überhangs in der Vorschau prüfen:**
Beide Slicer können Überhangs farblich markieren, noch bevor wir slicen.
In PrusaSlicer aktivieren wir die Overhang-Darstellung über das Dropdown
"View" in der Toolbar. In Cura erscheint die Overhang-Anzeige automatisch.
Rote beziehungsweise orange eingefärbte Flächen zeigen Bereiche, die mehr als
45 Grad zur Vertikalen überhängen und Stützstrukturen benötigen.

**Pass 1 - Standardorientierung beurteilen:**
...
Flächen wie die Führungsrille davon betroffen sind. Stützstrukturen an der
Rillenunterseite sind, wie in Abschnitt 7.1 erklärt, besonders unerwünscht,
weil sie dort eine raue Oberfläche hinterlassen.

**Pass 2 - Alternative Orientierungen testen:**
Wir drehen das Modell um 90 Grad um die Längsachse und prüfen die
Overhang-Anzeige erneut. In vielen Fällen wandern die kritischen Überhangs
von der Führungsrille an eine unkritische Außenfläche. Wir wählen die
Orientierung, bei der die Führungsrille am wenigsten Stützstrukturen benötigt.

**Pass 3 - Erste Schicht prüfen:**
...
**Fülldichte: 20 %**
Wir setzen "Infill density" auf 20 Prozent. Als Füllmuster empfehlen sich
"Gyroid" oder "Grid": Beide sind für Prototypen stabil genug. Grid ist
einfacher zu slicen und spart etwas Rechenzeit.

**Drucktemperatur: 205 °C und Bett: 60 °C**
Unter "Filament Settings" beziehungsweise den Temperatureinstellungen tragen
wir 205 °C für die Nozzle und 60 °C für das Druckbett ein. Diese Werte gelten
für PLA; für andere Materialien passen wir sie entsprechend der Tabelle aus
Abschnitt 7.1 an.
...
Stützstrukturen nur für Überhangs, die direkt bis zur Druckplatte reichen, und
vermeidet unnötige Stützstrukturen im Inneren des Objekts.

```{figure} pics/chap07_slicer_parameter.png
:alt: Slicer-Einstellungsfenster mit den fünf Schlüsselparametern: Schichthöhe
  0.15 mm, Fülldichte 20 Prozent, Nozzle-Temperatur 205 Grad, Druckgeschwindigkeit
  45 mm/s und aktivierte Stützstrukturen.
:align: center

Die fünf Schlüsselparameter im Einstellungsfenster des Slicers. Alle Werte
beziehen sich auf PLA als Druckmaterial.
(Quelle: eigene Abbildung; Lizenz [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0))
```
...
ob die Retraktionseinstellung aktiv ist.
...
kurzer Travel-Move (unter 5 mm) mit aktiver Retraktion erzeugt selten
sichtbares Stringing. Mehrere lange Travel-Moves (über 15 mm) ohne Retraktion
...
```{figure} pics/chap07_slicer_preview.png
:alt: Layer-Vorschau des Slicers mit farblich markierten Schichten,
  sichtbaren Stützstrukturen und einer hervorgehobenen Travel-Move-Linie
  über der Führungsrille.
:align: center

Layer-Vorschau nach dem Slicen: Stützstrukturen (grau), reguläre Schichten
(blau) und Travel-Moves (gestrichelt) sind farblich unterschieden. Ein langer
Travel-Move über die Führungsrille ist in dieser Ansicht gut erkennbar.
(Quelle: eigene Abbildung; Lizenz [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0))
```
...
direkt über die Führungsrille. Die Retraktionseinstellung ist auf 0.8 mm
gesetzt.

1. Warum ist dieser Travel-Move ein Stringing-Risiko, obwohl die Retraktion
   aktiviert ist?
...
:class: tip
:class: dropdown
1. Eine Retraktion von 0.8 mm ist für direkt angetriebene Extruder (Direct
   ...
   in der Nozzle auszugleichen, was bei zu niedriger Retraktionslänge zu
   ...
   trotz Retraktion auszutreten.

2. Erstens: Retraktionslänge auf 1.5 bis 2.0 mm erhöhen (bei Bowden-Extruder
...
```
; generated by PrusaSlicer 2.8 on ...
; layer_height = 0.15
; infill_density = 20%
...
* **Layer-Vorschau geprüft:** Keine langen Travel-Moves ohne Retraktion über
...
```{dropdown} Video "How to Use a Slicer: Complete Beginner's Guide" von Printables
