...
Für die Weiterverarbeitung in CloudCompare ist die Texturauflösung weniger kritisch – dort arbeiten wir hauptsächlich mit der Geometrie. Für die abschließende Dokumentation und den Präsenztag lohnt sich jedoch die höhere Auflösung.
...
1. DepthMap → downscale = 4
   Einbuße: Gröbere Tiefenkarten, weniger feine Details in der Punktwolke

2. Meshing → maxInputPoints = 5 000 000 (statt 50 000 000)
   Einbuße: Gröberes Mesh, weniger Dreiecke, feine Strukturen können verloren gehen

3. Texturing → textureSide = 2048 (statt 4096)
   Einbuße: Niedrigere Texturauflösung, Detailverlust in der Oberflächenfarbe
...
:class: tip, dropdown
```
1. Watertight: False bedeutet, dass das Mesh nicht vollständig geschlossen ist –
   es hat Lücken oder non-manifold Kanten. Es beschreibt kein eindeutiges Volumen.

2. Für die Arbeit in CloudCompare ist das kein unmittelbares Problem: Wir können
   das Mesh analysieren, bereinigen und glätten, auch wenn es noch nicht watertight ist.
   CloudCompare arbeitet direkt mit der Geometrie, ohne ein geschlossenes Volumen vorauszusetzen.

3. Watertight: True wird für den 3D-Druck in PrusaSlicer benötigt. Der Slicer
   muss Innen und Außen eindeutig unterscheiden können. Deshalb reparieren wir
   das Mesh in CloudCompare (Kapitel 4–6), bevor wir es an PrusaSlicer übergeben.
```
````
