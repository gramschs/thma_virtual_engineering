...
Für unsere Kugelbahn empfehlen wir für einen ersten Testdurchlauf reduzierte
Werte (zum Beispiel `maxInputPoints = 10 000 000`), um schnell ein beurteilbares
Ergebnis zu erhalten. Für das finale Mesh, das in den 3D-Druck gehen soll,
erhöhen wir die Werte wieder.
...
:class: tip
Sie starten einen Meshroom-Durchlauf mit dem Musterdatensatz auf einem Laptop
ohne dedizierte GPU. Die Berechnung würde bei Standardparametern ca. 4 Stunden
dauern.
...
nur den Parameter `textureSide` ändern, müssen wir nicht die gesamte Pipeline
von vorn starten, nur der Texturing-Knoten muss neu berechnet werden. Meshroom
erkennt automatisch, welche Zwischenergebnisse noch gültig sind.
